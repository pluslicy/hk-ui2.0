<template>
  <div class="lightRoom">
    <!-- 水箱液位仪 -->
    <!-- 水箱液位仪图片 -->
    <div class="lightRoom_img">
      <img :src="room.room_plane_imgpath" alt="图片加载中..." style="height:100%;width:100%">
      <canvas id="lightRoomCanvas" :width="canvasWidth" height="250px" />
    </div>
    <!-- 水箱液位仪数据 -->
    <div class="lightRoom_data">
      <el-table
        v-loading="loading"
    ...
    <!-- 查询历史数据 -->
    <div class="lightRoom_history">
      <div class="lightRoom_history_form">
      ...
      <div class="lightRoom_history_data">
        <div id="lightRoom" :style="he" />
      </div>
    </div>
  </div>
</template>
...
export default {
  ...
  created() {
    ...
    // 查询时间改变
    this.queryChange()
    // this.lightRoom = this.$parent.device
    // 加载所有的设备
    this.findRoomAllDeviceOfOne()
    this.findAllRoom()
    this.currentDate()
  },
  destroyed(){
  ...
  methods: {
    ...
    drawCanvas() {
      const vm = this
      const canvas = document.getElementById('lightRoomCanvas')
      ...
      this.roomAllDeviceOfOne.forEach((item) => {
    ...
    drawlightRoom(data) {
      // console.log(data)
      // 得到时间
      var lightRoomTime = data.times.map((item) => {
        return item.split(' ')[1]
      })
      // console.log(lightRoomTime)
      // 基于准备好的dom，初始化echarts实例
      var myChart = echarts.init(document.getElementById('lightRoom'))
      // 指定图表的配置项和数据
      var option = {
        tooltip: {
          trigger: 'axis'
...
.lightRoom_history .lightRoom_history_form .dataOptions {
  cursor: pointer;
  margin-right: .5em;
  color: #fff;
  border-radius: 3px;
...
.lightRoom_history .lightRoom_history_form .optionDay {
  background: #409eff;
}
.lightRoom_history .lightRoom_history_form .optionWeek {
  background: #409eff;
}
.lightRoom_history .lightRoom_history_form .optionMonth {
  background: #409eff;
}
.lightRoom_history .lightRoom_history_form .optionQuarter {
  background: #409eff;
}
</style>
