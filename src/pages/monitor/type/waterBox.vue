<template>
  <div class="waterBox">
    <!-- 水箱液位仪 -->
    <!-- 水箱液位仪图片 -->
    <div class="waterBox_img">
      <img :src="room.room_plane_imgpath" alt="图片加载中..." style="height:100%;width:100%">
      <canvas id="waterBoxCanvas" :width="canvasWidth" height="250px" />
    </div>
    <!-- 水箱液位仪数据 -->
    <div class="waterBox_data">
      <el-table
        v-loading="loading"
    ...
    <!-- 查询历史数据 -->
    <div class="waterBox_history">
      <div class="waterBox_history_form">
      ...
      <div class="waterBox_history_data">
...
export default {
  ...
  created() {
    ...
    // 查询时间改变
    this.queryChange()
    // this.waterBox = this.$parent.device
    // 加载所有的设备
    this.findBoxAllDeviceOfOne()
    this.findAllRoom()
    this.currentDate()
  },
  destroyed(){
  ...
  methods: {
    ...
    drawCanvas() {
      const vm = this
      const canvas = document.getElementById('waterBoxCanvas')
      ...
      this.boxAllDeviceOfOne.forEach((item) => {
    ...
    drawwaterBox(data) {
      // console.log(data)
      // 得到时间
      var waterBoxTime = data.times.map((item) => {
        return item.split(' ')[1]
      })
      // console.log(waterBoxTime)
      // 基于准备好的dom，初始化echarts实例
      var myChart = echarts.init(document.getElementById('waterBox'))
      // 指定图表的配置项和数据
      var option = {
        tooltip: {
          trigger: 'axis'
...
.waterBox_history .waterBox_history_form .dataOptions {
  cursor: pointer;
  margin-right: .5em;
  color: #fff;
  border-radius: 3px;
...
.waterBox_history .waterBox_history_form .optionDay {
  background: #409eff;
}
.waterBox_history .waterBox_history_form .optionWeek {
  background: #409eff;
}
.waterBox_history .waterBox_history_form .optionMonth {
  background: #409eff;
}
.waterBox_history .waterBox_history_form .optionQuarter {
  background: #409eff;
}
</style>
