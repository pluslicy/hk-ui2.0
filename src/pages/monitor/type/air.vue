<template>
  <div class="airBox">
    <!-- 空气 -->
    <!-- 空气图片 -->
    <div class="airBox_img">
      <img :src="room.room_plane_imgpath" alt="图片加载中..." style="height:100%;width:100%">
      <canvas id="airBoxCanvas" :width="canvasWidth" height="250px" />
    </div>
    <!-- 空气数据 -->
    <div class="airBox_data">
      <el-table
        v-loading="loading"
    ...
    <!-- 查询历史数据 -->
    <div class="airBox_history">
      <div class="airBox_history_form">
      ...
      <div class="airBox_history_data">
        <div id="airBox" :style="he" />
      </div>
    </div>
  </div>
</template>
...
export default {
  ...
  created() {
    ...
    // 查询时间改变
    this.queryChange()
    // this.airBox = this.$parent.device
    // 加载所有的设备
    this.findAirAllDeviceOfOne()
    this.findAllRoom()
    this.currentDate()
  },
  destroyed(){
  ...
  methods: {
    ...
    drawCanvas() {
      const vm = this
      const canvas = document.getElementById('airBoxCanvas')
    ...
    drawairBox(data) {
      // console.log(data)
      // 得到时间
      var airBoxTime = data.times.map((item) => {
        return item.split(' ')[1]
      })
      // console.log(airBoxTime)
      // 基于准备好的dom，初始化echarts实例
      var myChart = echarts.init(document.getElementById('airBox'))
      // 指定图表的配置项和数据
      var option = {
        tooltip: {
          trigger: 'axis'
...
.airBox_history .airBox_history_form .dataOptions {
  cursor: pointer;
  margin-right: .5em;
  color: #fff;
  border-radius: 3px;
...
.airBox_history .airBox_history_form .optionDay {
  background: #409eff;
}
.airBox_history .airBox_history_form .optionWeek {
  background: #409eff;
}
.airBox_history .airBox_history_form .optionMonth {
  background: #409eff;
}
.airBox_history .airBox_history_form .optionQuarter {
  background: #409eff;
}
</style>
